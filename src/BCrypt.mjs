...
function hhash(s, salt, callback, progressCallback) {

    function _async(callback) {
        if (typeof s === 'string' && typeof salt === 'number')
            generateSalt(salt, function(err, salt) {
 ...
 * @inner
 */
function internalCrypt(b, salt, rounds, callback, progressCallback) {
...
function internalHash(s, salt, callback, progressCallback) {
    var err;
    ...
    // Sync
    if (typeof callback == "undefined") {
        return finish(internalCrypt(passwordb, saltb, rounds));
    }

    // Async
    else {
        internalCrypt(passwordb, saltb, rounds, function(err, bytes) {
            if (err)
                callback(err, null);
            else
...
function internalEksKey(data, key, P, S) {
    var offp = 0,
        lr = [0, 0],
        plen = P.length,
        slen = S.length,
        sw;
    for (var i = 0; i < plen; i++)
        sw = streamToWword(key, offp),
        offp = sw.offp,
        P[i] = P[i] ^ sw.key;
    offp = 0;
    for (i = 0; i < plen; i += 2)
        sw = streamToWword(data, offp),
        offp = sw.offp,
        lr[0] ^= sw.key,
        sw = streamToWword(data, offp),
        offp = sw.offp,
